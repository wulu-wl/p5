...
Db.prototype = {
	...
	find(collection_name,obj,backFn){
		obj.query = obj.query || {}   //{name:"shao"}
		obj.skip = obj.skip || 0;   //跳过多少条数据
		obj.limit = obj.limit || 0; //limit值为0是代表所有数据  限制得到多条
		this.connect((dbase)=>{
			dbase.collection(collection_name)
			.find(obj.query)
			.skip(obj.skip)
...
//导出的是一个构造函数   所以在引用的时候需要使用new关键字实例化对象
module.exports = Db;
